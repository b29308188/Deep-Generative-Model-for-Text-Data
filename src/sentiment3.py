...
class Generator:
    def __init__(self, max_len = 300, hidden_dim = 32):
        ...
        self.D.compile(loss = "categorical_crossentropy", optimizer = Adam(lr=0.00001))
    def build_generative_model(self):
        G = Sequential()
	G.add(LSTM(32, input_dim = self.hidden_dim, input_length = self.max_len, return_sequences = True))
        return G
...
if __name__ == "__main__":
    ...
    posX = pos_gan.generate((10, 300, 32)).sum(axis = 1) 
    pos_gan.save("../models/pos_rnn_pretrain")
    ...
    negX = neg_gan.generate((10, 300, 32)).sum(axis = 1)    
    neg_gan.save("../models/neg_rnn_pretrain")
    
    pos_gan = Generator()    
    neg_gan = Generator()    
    pos_gan.load("../models/pos_rnn_nopretrain")
    neg_gan.load("../models/neg_rnn_nopretrain")
    #posX = pos_gan.generate((10, 300, 32)).sum(axis = 1) 
    #print posX
    #print trainX_pos
    #negX = neg_gan.generate((10, 300, 32)).sum(axis = 1)    
    ...
    trainX = trainX.sum(axis = 1)
    testX = testX.sum(axis = 1)
